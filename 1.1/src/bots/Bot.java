...
public class Bot {
	...
	private void updateLEDs() {
		/* 
		 * if roaming, turn all LEDs OFF
		 * if connected to a bot and that bot is not yet GREEN, turn BLUE
		 * if waiting for bot, turn RED and WHITE in directions of pending connection
		 * if bots are connected to all white LEDs, turn GREEN 
		 */
	...
	/* RETURNS TRUE IF THERE IS A CLOSE BOT THAT HAS A RED LED ON (MEANING CONNECTION CONFIRMATION) */
	public boolean botHasConnected() {
		for (int i = 0; i < closeBots.size(); i++) {
			if (closeBots.get(i).LEDManager.hasLEDSetTo(Color.RED)) {
				System.out.println("BOT CONNECTED, TURNING LEDS OFF");
				return true;
			}
		}
		return false;
