...
public class Sensors {
	...
	protected double getRelativeDirectionTo(Bot bot) {
		double dir = Math.abs(getDirectionTo(bot) - getDirection());
	...
	/* RETURNS THE CLOSEST BLUE LED, RETURNS NULL IF NONE FOUND */
	protected LED getClosestBlueLED() {
		ArrayList<Bot> closeBots = calcAndGetCloseBots(100.0D);
		LED closestLEDSoFar = null;
		// go through all bots seen
		for (int i = 0; i < closeBots.size(); i++) {
	...
	protected void turnOffWhiteLEDsIfBlueBotConnected() {
		...
		LED closestBlueLED = getClosestBlueLED();
		if (closestBlueLED == null) { return; }
		
		// if that BLUE LED is too far away to be connected to us, return
		if (getDistanceTo(closestBlueLED) > getBotSize()*2) { return; }
		...
		for (int i = 0; i < currentBot.LEDs.size(); i++) {
			if (currentBot.LEDs.get(i).getColorString() == "white") {		
				if (isInDirection(closestBlueLED.getOwner(), currentBot.LEDs.get(i).getDirToParentBot())) {
					currentBot.LEDs.get(i).setColor(Color.RED);
				} else {
					System.out.println("See blue LED but not yet in direction.");
				}
			}
		}
	}
	...
	protected boolean isInDirection(Bot bot, int dir) {
		dir = fixDirection(dir);
		if ((this.getRelativeDirectionTo(bot) < (dir + 45)) && (this.getRelativeDirectionTo(bot) > (dir - 45))) {
			return true;
		}
		return false;
	}
