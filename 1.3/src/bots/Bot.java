/*
 * This is the main Bot class. It contains various other classes such as the sensors, an LEDManager, and its LEDs to work together and decide what this bot should do next.
 */
...
public class Bot {
	...
	private double charge;	// the charge of the bot
	private int tickCount;	// used to randomise roaming. Every x ticks the bot turns in a random direction
	private boolean isActive;	// true = the bot is active
	private ArrayList<Bot> seenBots;	// a list of all bots that the bot can see
	private ArrayList<Bot> closeBots;	// a list of all bots that are close to the bot
	private ArrayList<Object> seenCharges;	// a list of seen bots and LEDs, used to calculate VFF
	public ArrayList<LED> LEDs;	// containts pointers to all the LEDs on the bot (going clockwise from the front of the bot)
	...
	private double desiredDirection;	// the direction the bot is aiming to get to, NOT the actual facing direction
	private static final double TURNSPEED = 5.0;	// speed at which the bot turns
	...
	private void renderBot(Graphics g) {
		...
		xLineStart = (int) xPos;
		yLineStart = (int) yPos;
		xLineEnd   = (int) (xLineStart + Math.cos(Math.toRadians(desiredDirection)) * d / 4);
		yLineEnd   = (int) (yLineStart + Math.sin(Math.toRadians(desiredDirection)) * d / 4);
